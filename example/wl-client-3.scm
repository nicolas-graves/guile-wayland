...
(define-record-type <mmap>
  (%make-mmap pointer bytevector)
  mmap?
  (pointer mmap-pointer)
  (bytevector mmap-bytevector))
...
(define mmap
  (let ((% (foreign-library-function
    ...
    (lambda (address length prot flags fd offset)
      (let ((p (% (or address ffi:%null-pointer) length prot flags fd offset)))
        (%make-mmap
         p
...
(define munmap
  (let ((% (foreign-library-function
    ...
    (lambda* (mm)
      (% (mmap-pointer mm)
         (bytevector-length (mmap-bytevector mm))))))
...
(define (draw-frame shm)
  (let* ((width (width*))
         ...
         (mm (mmap #f size (logior PROT_READ PROT_WRITE) MAP_SHARED fd 0))
         (data (mmap-bytevector mm))
